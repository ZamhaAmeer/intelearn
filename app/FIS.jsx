...
// Update LessonItem to receive an onPress callback trigger
...
export default function CourseScreen() {
 ...
 useEffect(() => {
  const fetchCourseData = async () => {
    try {
      // Pull down the signed session token safely from device cache memory
      ...
      if (!response.ok) throw new Error('Failed to extract interactive course payload');

      const data = await response.json();
      setCourseData(data);
    } catch (error) {
      console.error("Error fetching course data content:", error);
    } finally {
      setLoading(false);
    }
  };
            ...
            {courseData.materials && courseData.materials.length > 0 ? (
              courseData.materials.map((material, index) => (
                <LessonItem 
                  ...
                  onPress={() => {
                    // Construct standard web stream endpoints resolving to your static Express upload directories
                    const rawUrl = `http://172.20.10.3:3000/${material.material_url}`;
                    
                    // Push straight into your learning module player UI (e.g., PDF viewer screen)
                    router.push({
                      pathname: '/pdfviewer', // Make sure this matches your PDF Viewer file path
                      params: { 
