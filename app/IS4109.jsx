...
export default function CourseScreen() {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      const targetCode = code || 'IS4109';
      try {
        const token = await AsyncStorage.getItem('token');

        // Hits the new code lookup route
        ...
        if (!response.ok) throw new Error('Failed to extract course payload');
        ...
        setCourseData({
          ...
          code: 'IS4109',
          description: 'This course covers the fundamentals of Agile software development methodologies, including Scrum, Kanban, sprints, user stories, and continuous integration/delivery pipelines.',
          materials: [
            {
              id: 1001,
              title: 'Introduction to Agile & Scrum Framework',
              material_url: 'uploads/sample.pdf'
            },
            {
              id: 1002,
              title: 'User Stories & Agile Estimation',
              material_url: 'uploads/sample.pdf'
            },
            {
              id: 1003,
              title: 'Kanban Board & Workflow Optimization',
              material_url: 'uploads/sample.pdf'
            }
          ]
        });
  ...
  if (loading) {
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={{ marginTop: 10, color: '#4E33B3', fontWeight: '600' }}>Syncing Course Catalog...</Text>
      </View>
    );
  }
    ...
    <View style={[styles.popupContentArea, isDark && { backgroundColor: "#1E1E1E" }]}>
      <View style={styles.notifListItem}>
        ...
        <View style={styles.notifTextContainer}>
          ...
          <Text style={[styles.notifDesc, isDark && { color: "#AAAAAA" }]}>Structured Programming: Unit 4 - Logic Gates</Text>
      ...
      <View style={styles.notifListItem}>
        ...
        <View style={styles.notifTextContainer}>
          <Text style={[styles.notifTitle, isDark && { color: "#FFFFFF" }]}>Exam date reminder</Text>
          <Text style={[styles.notifDesc, isDark && { color: "#AAAAAA" }]}>Fundamentals of IS mid-term scheduled for Oct 12th.</Text>
    ...
    <View style={[styles.popupContentArea, isDark && { backgroundColor: "#1E1E1E" }]}>
      ...
      <View style={[styles.calEventCard, isDark && { backgroundColor: "#2A2A2A" }]}>
        ...
        <View style={styles.calEventInfo}>
          ...
          <Text style={[styles.calEventTitle, isDark && { color: "#FFFFFF" }]}>IS Final Review Session</Text>
        </View>
      </View>
    </View>
  );
    ...
    <View style={[styles.container, isDark && { backgroundColor: '#090D16' }]} {...panResponder.panHandlers}>
      ...
      >
        ...
        <View style={styles.contentBody}>
          ...
          {/* SAFE FALLBACK FOR EMPTY OR NULL TEXT DESCRIPTIONS */}
          ...
          {/* Shared Notes Card */}
          <TouchableOpacity
            style={styles.notesSectionButton}
            onPress={() => {
              router.push({
                pathname: '/student_notes',
                params: { courseCode: courseData.code || 'IS4109' }
              });
            }}
          >
            <View style={styles.notesButtonLeft}>
              <View style={styles.notesIconCircle}>
                <Icon name="folder-open" size={24} color="#4E33B3" />
              </View>
              <View style={{ marginLeft: 15, flex: 1 }}>
                <Text style={styles.notesButtonTitle}>Lecture Notes & Slides</Text>
                <Text style={styles.notesButtonSubtitle}>Access shared PDFs and PPTX files</Text>
              </View>
            </View>
            <ChevronRight color="#4E33B3" size={20} />
          </TouchableOpacity>
                            ...
                            AsyncStorage.getItem('role').then(role => {
                              if (role === 'lecturer') {
                                if (!isAssignment) {
                                  ...
                                  alert('Assignment details tracking is not yet implemented for lecturers.');
                                }
                              } else {
                                if (isAssignment) {
          ...
          {/* RENDERING DYNAMIC DATABASE DRIVEN MATERIALS LESSON LIST */}
          {courseData.materials && courseData.materials.length > 0 ? (
            courseData.materials.map((material, index) => (
              <LessonItem
                ...
                onPress={() => {
                  const safeUrl = material.material_url ? material.material_url.replace(/\\/g, '/') : '';
      ...
      {/* REFACTORED CLEAN TABBED MODAL POPUP (COMBINED CALENDAR & NOTIFICATIONS) */}
        ...
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={toggleMenu}>
          <View style={[styles.sideMenu, isDark && { backgroundColor: '#1A1A1A' }]}>
            ...
            <View style={styles.menuList}>
              ...
              <MenuOption iconName="folder-open" title="Shared Notes" onPress={() => { setMenuVisible(false); router.push({ pathname: '/student_notes', params: { courseCode: courseData?.code || 'IS4109' } }) }} />
...
// Keep your styles exactly the same as you had them down here...
